    ...
    <div class="modal-backdrop" data-action="close-modal">
      <article class="modal-card compact-modal" aria-labelledby="evTypologyTitle">
        ...
        <form id="evTypologyForm" class="modal-body" data-record-id="${escapeAttribute(record.id)}">
          <label class="field"><span>Tipologia</span><input name="typology" list="evUnifiedTypologyOptions" required value="${escapeAttribute(record.typology)}" placeholder="Selecione ou digite uma nova tipologia" /></label>
          <datalist id="evUnifiedTypologyOptions">${options.map((option) => `<option value="${escapeAttribute(option)}"></option>`).join("")}</datalist>
