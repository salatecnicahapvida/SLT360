...
function enhanceSortableTables() {
  document.querySelectorAll("table.data-table").forEach((table, tableIndex) => {
    ...
    table.querySelectorAll("thead th").forEach((header, columnIndex) => {
      ...
      header.innerHTML = `<button type="button" data-action="sort-generic-table" data-column-index="${columnIndex}" title="Clique: maior para menor; segundo clique: menor para maior; terceiro clique: ordem original"><span>${escapeAttribute(label)}</span><i>↕</i></button>`;
    });
  });
}
...
function renderWorksStrategic() {
  ...
  const typologyCostSamples = new Map();
  validAreaRecords.forEach((record) => {
    const key = record.typology || "Não informada";
    const samples = typologyCostSamples.get(key) || [];
    samples.push(Number(record.total) / Number(record.area));
    typologyCostSamples.set(key, samples);
  });
  const typologyCostBenchmarks = new Map([...typologyCostSamples.entries()].map(([key, values]) => [key, median(values)]));
  const globalCostBenchmark = median(validAreaRecords.map((record) => Number(record.total) / Number(record.area)));
    ...
    <section class="panel strategic-investment-panel strategic-intelligence-ranking">
      <div class="panel-header">
        <div>
          ...
          <p class="panel-subtitle">Valor, concentração e desvio do R$/m² comparados à mediana da própria tipologia</p>
        </div>
        <div class="inline-actions">
      ...
      <div class="table-wrap strategic-intelligence-table-wrap">
        <table class="data-table strategic-intelligence-table">
          ...
          <tbody>
            ${investmentRows.slice(0, 15).map((row, index) => {
              const costM2 = Number(row.record.area) > 0 ? row.valor / Number(row.record.area) : 0;
              const typologyKey = row.record.typology || "Não informada";
              const manualBenchmark = Number(state.evReferenceTargets?.[typologyKey]) || 0;
              const benchmark = manualBenchmark || typologyCostBenchmarks.get(typologyKey) || globalCostBenchmark || 0;
              ...
              let reading = "Dentro da referência";
              let tone = "green";
              if (!costM2) { reading = "Sem área para validar R$/m²"; tone = "gray"; }
              else if (Math.abs(deviation) >= 30) { reading = `${deviation > 0 ? "Acima" : "Abaixo"} da mediana em ${number(Math.abs(deviation), 0)}%`; tone = "red"; }
              else if (Math.abs(deviation) >= 15) { reading = `Variação de ${deviation > 0 ? "+" : "-"}${number(Math.abs(deviation), 0)}%`; tone = "orange"; }
              else if (share >= 5) { reading = `Alta concentração: ${number(share, 1)}% da base`; tone = "blue"; }
              return `<tr><td class="numeric"><strong>${index + 1}</strong></td><td><strong>${escapeAttribute(row.record.project)}</strong><br><span class="muted">${escapeAttribute(String(row.record.year))} · ${escapeAttribute(row.record.typology)} · ${escapeAttribute(row.record.code || "Sem código")}</span></td><td class="numeric"><strong>${moneyCompact(row.valor)}</strong><br><span class="muted">${money(row.valor)}</span></td><td class="numeric">${number(share, 1)}%</td><td class="numeric">${costM2 ? `${money(costM2)}/m²` : "—"}<br><span class="muted">${row.record.area ? `${number(row.record.area, 0)} m²` : "Sem área"}</span></td><td class="numeric">${benchmark ? `${money(benchmark)}/m²` : "—"}<br><span class="muted">${manualBenchmark ? "Meta ajustada" : "Mediana automática"}</span></td><td><span class="executive-reading" data-tone="${tone}">${reading}</span></td><td><button class="ghost-button compact-action" type="button" data-action="${openAction}" data-id="${escapeAttribute(openId)}">Ver EV</button></td></tr>`;
            }).join("")}
          </tbody>
        </table>
      </div>
...
function openEVReferenceTargetsModal() {
  const records = evUnifiedRecords().filter((record) => Number(record.total) > 0 && Number(record.area) > 0);
  const groups = new Map();
  records.forEach((record) => {
    const key = record.typology || "Não informada";
    const values = groups.get(key) || [];
    values.push(Number(record.total) / Number(record.area));
    groups.set(key, values);
  });
  const rows = [...groups.entries()]
    .map(([typology, values]) => ({ typology, median: median(values), count: values.length }))
    ...
    <div class="modal-backdrop" data-action="close-modal">
      <section class="modal-card ev-reference-modal" role="dialog" aria-modal="true" aria-labelledby="evReferenceTitle" data-modal-panel>
        <header class="modal-header">
          <div><span class="eyebrow">INTELIGÊNCIA DE CUSTO</span><h2 id="evReferenceTitle">Metas de referência por tipologia</h2><p>Defina o R$/m² esperado. Campos vazios continuam usando a mediana automática da base histórica.</p></div>
          <button class="icon-button" type="button" aria-label="Fechar" data-action="close-modal">×</button>
        </header>
        <div class="reference-guidance"><strong>Como funciona</strong><span>A meta ajustada passa a ser a referência dos alertas executivos. A mediana histórica permanece visível para comparação.</span></div>
        <div class="table-wrap ev-reference-table-wrap">
          <table class="data-table ev-reference-table">
            <thead><tr><th>TIPOLOGIA</th><th class="numeric">EVS VÁLIDOS</th><th class="numeric">MEDIANA HISTÓRICA</th><th>META AJUSTÁVEL (R$/M²)</th></tr></thead>
            <tbody>${rows.map((row) => `<tr><td><strong>${escapeAttribute(row.typology)}</strong></td><td class="numeric">${row.count}</td><td class="numeric">${money(row.median)}/m²</td><td><input class="reference-target-input" type="number" min="0" step="1" data-reference-typology="${escapeAttribute(row.typology)}" value="${Number(state.evReferenceTargets?.[row.typology]) || ""}" placeholder="Automática: ${number(row.median, 0)}" aria-label="Meta de ${escapeAttribute(row.typology)} em reais por metro quadrado"></td></tr>`).join("")}</tbody>
          </table>
        </div>
...
function saveEVReferenceTargets() {
  const targets = {};
  modalRoot.querySelectorAll("[data-reference-typology]").forEach((input) => {
    const value = Number(input.value);
    if (Number.isFinite(value) && value > 0) targets[input.dataset.referenceTypology] = value;
  });
  state.evReferenceTargets = targets;
  saveState();
  closeModal();
  render();
  ...
  return `
    <section class="panel ev-history-panel">
      ...
      <div class="ev-history-grid">
        ...
        <article class="ev-history-table-card"><div class="panel-header"><div><h3>Carteira unificada de EVs</h3><p class="panel-subtitle">Clique nos títulos das colunas: maior → menor, menor → maior e ordem original.</p></div><span class="tag">${records.length} EVs</span></div><div class="table-wrap ev-history-table-wrap"><table class="data-table ev-unified-table"><thead><tr>${evSortableHeader("Ano", "year")}${evSortableHeader("EV", "project")}${evSortableHeader("Tipologia", "typology")}${evSortableHeader("Valor", "total", true)}${evSortableHeader("Área", "area", true)}${evSortableHeader("% disciplina", "discipline", true)}<th>Ação</th></tr></thead><tbody>
