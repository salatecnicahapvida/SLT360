...
function normalizeDemandLabels(labels = []) {
  const values = Array.isArray(labels) ? labels : String(labels || "").split(/[,;\n]+/);
  const normalized = values
    .map((label) => String(label || "").trim().slice(0, 32))
    .filter(Boolean);
  return [...new Map(normalized.map((label) => [normalizeSearchText(label), label])).values()].slice(0, 6);
}
...
function exportWorksOperationalReport() {
  const rows = filteredDemands().map((demand) => {
    ...
    return [
      ...
      sicInfo.lecomNumber || "",
      sicInfo.tituloSic || "",
      demandHasRecordedValue(demand) ? money(demandProducedValue(demand)) : "",
      demand.observacao || "",
    ];
  });
  downloadCsv(
                  ...
                  <tr data-action="open-project-plan-detail" data-row="${row.row}" role="button" tabindex="0">
                    ...
                    <td>
                      <div class="table-actions">
                        ${row.obraId ? `<button class="secondary-action compact-action" type="button" data-action="start-budget-from-plan" data-row="${row.row}">Enviar para Obras</button>` : `<button class="secondary-action compact-action" type="button" data-action="open-work-from-plan" data-row="${row.row}">Cadastrar</button>`}
                      </div>
                    </td>
                  </tr>
                `;
...
function renderProjectCard(row) {
  ...
  return `
    <article class="demand-card project-demand-card" data-status="${status.id}" data-action="open-project-plan-detail" data-row="${row.row}" role="button" tabindex="0">
      ...
      ${budgetDemand ? `<div class="demand-card-unit-badge">Demanda de Obras criada · ${escapeAttribute(budgetDemand.id)}</div>` : ""}
        ...
        <div class="modal-body">
          ...
          <form class="project-status-form" data-project-status-form>
            ...
            <div class="project-status-actions">
              ...
              ${budgetDemand ? `<button class="secondary-action" type="button" data-action="open-demand-detail" data-id="${budgetDemand.id}">Abrir card em Obras</button>` : ""}
            </div>
          </form>
        </div>
        <footer class="modal-actions">
          ${row.customProjectDemand ? "" : budgetDemand ? `<button class="primary-action" type="button" data-action="open-demand-detail" data-id="${budgetDemand.id}">Abrir Obras</button>` : work ? `<button class="primary-action" type="button" data-action="create-budget-from-project" data-row="${row.row}">Criar card em Obras</button>` : `<button class="primary-action" type="button" data-action="open-work-from-plan" data-row="${row.row}">Cadastrar em Obras</button>`}
...
function filteredDemands() {
  return state.demands.filter((demand) => {
    ...
    const text = normalizeSearchText(
      `${demand.id} ${work?.nome || ""} ${demandAnalystNames(demand).join(" ")} ${normalizeDemandLabels(demand.etiquetas).join(" ")} ${demand.tipo} ${demand.observacao || ""} ${sicInfo.lecomNumber || ""} ${sicInfo.numeroSic || ""} ${sicInfo.tituloSic || ""} ${sicInfo.obraNumber || ""} ${sicInfo.obraNome || ""}`
    );
...
function renderDemandCardLabels(labels = []) {
  const normalized = normalizeDemandLabels(labels);
  if (!normalized.length) return "";
  return `<div class="demand-card-labels" aria-label="Etiquetas do card">${normalized
    .map((label) => `<span>${escapeAttribute(label)}</span>`)
    .join("")}</div>`;
}

function renderDemandLabelsField(labels = []) {
  return `
    <label class="field full-span demand-labels-field">
      <span>Etiquetas do card</span>
      <input name="etiquetas" value="${escapeAttribute(normalizeDemandLabels(labels).join(", "))}" placeholder="Ex.: Urgente, Diretoria, Fornecedor" maxlength="200" />
  ...
  return `
    <section class="panel sprint-settings-panel">
      <div class="panel-header">
        <div>
          <h2>Sprints globais</h2>
          <p class="panel-subtitle">Cadastro único usado nas visões operacionais de Obras, Manutenção, Engenharia Clínica e Controle de Verbas</p>
        </div>
...
function openDemandWizardModal(type = "EmissaoInicial", step = 1, draft = {}) {
  ...
  modalRoot.innerHTML = globalThis.SLT_CLOUD.cleanHTML(step === 1 ? renderDemandWizardStep1(demandWizardDraft) : renderDemandWizardStep2(demandWizardDraft));
}
...
function demandWizardDefaultDraft(type, draft = {}) {
  ...
  return {
    ...
    sprintId: sprint?.id || "",
    analistaResponsavel: analystAssignment.analistaResponsavel || suggestedAnalyst,
    analistasComplementares: analystAssignment.analistasComplementares,
    etiquetas: normalizeDemandLabels(draft.etiquetas),
      ...
      <form class="modal-card demand-modal-card demand-detail-form demand-create-card demand-wizard-card" id="demandForm" aria-labelledby="demandTitle">
        ...
        <div class="modal-body">
          ...
          <section class="modal-section">
            ...
            ${renderDemandLabelsField(draft.etiquetas)}
          </section>
...
function historicalRecordForWork(work) {
  if (!work) return null;
  const records = arrayOrFallback(state.evs);
  const sourceId = work.historicalRecordId || work.sourceHistoricalRecordId;
  return records.find((record) => String(record.id) === String(sourceId || ""))
    || records.find((record) => String(record.workId || "") === String(work.id || ""))
    || records.find((record) => evUnifiedWorkForHistorical(record)?.id === work.id)
    || null;
}

function historicalAnalystForWork(work) {
  const analyst = String(historicalRecordForWork(work)?.technician || "").trim();
  return analyst && !["nao informado", "sem tecnico", "-", "—"].includes(normalizeSearchText(analyst)) ? analyst : "";
}
        ...
        <div class="modal-body">
          ...
          <div class="form-grid">
            ...
            <label class="field">
              ...
              <input name="dataPrevistaEntrega" type="date" value="${addDaysISO(todayISO(), 15)}" required />
            </label>
            <label class="field">
              <span>Motivo</span>
              <select name="motivo">
...
async function handleDemandSubmit(form) {
  ...
  state.demands.unshift({
    ...
    dataPrevistaEntrega: formData.get("dataPrevistaEntrega") || (tipo === "SIC" ? addDaysISO(todayISO(), 15) : ""),
...
async function handleDemandDetailSubmit(form) {
  ...
  demand.etiquetas = normalizeDemandLabels(formData.get("etiquetas"));
  if (isSicDemand) {
  ...
  if (JSON.stringify(previousLabels) !== JSON.stringify(demand.etiquetas)) {
    addHistory({
      entidade: "demanda",
      entidadeId: demand.id,
      campo: "etiquetas",
