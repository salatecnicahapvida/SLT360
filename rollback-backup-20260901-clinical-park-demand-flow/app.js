...
function applyClinicalEquipmentToForm(form, asset) {
  ...
  if (results) results.innerHTML = `<div class="selected-work-card"><strong>${escapeAttribute(asset.tag || asset.id)} · ${escapeAttribute(asset.equipamento)}</strong><span>${escapeAttribute(asset.unidadeNome)} · ID ${escapeAttribute(asset.id)}</span><small>${escapeAttribute([asset.fabricante, asset.modelo, asset.numeroSerie && `Série ${asset.numeroSerie}`].filter(Boolean).join(" · "))}</small></div>`;
}
...
function renderClinicalAssetRegistry(assets) {
  ...
  return `
    <div class="table-wrap">
      ...
      <p class="muted">Mostrando 120 de ${number(clinicalEquipmentRecords().length || assets.length)} equipamentos. Use a busca da nova demanda para localizar qualquer TAG ou ID da base.</p>
    </div>
  `;
}
...
document.addEventListener("input", (event) => {
  if (event.target.matches("[data-clinical-asset-search]")) {
    ...
    if (hidden) hidden.value = "";
    const results = form?.querySelector("[data-clinical-equipment-results]");
    if (results) results.innerHTML = clinicalEquipmentSearchResults(event.target.value);
    return;
  }
