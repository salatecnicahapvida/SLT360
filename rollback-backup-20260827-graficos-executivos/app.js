...
function strategicHeroMetric(label, value, detail = "") {
  return `
    <article class="strategic-hero-metric">
      <small>${label}</small>
      <strong>${value}</strong>
  ...
  return `
    ...
    <section class="strategic-hero panel">
      ...
      <div class="strategic-hero-metrics">
        ${strategicHeroMetric("Sala Técnica total", moneyCompact(capex), money(capex))}
        ${strategicHeroMetric("Maior obra negociada", topInvestment ? topInvestment.record.nomeObra : "Sem carteira", topInvestment ? `${moneyCompact(topInvestment.valor)} · ${number((topInvestment.valor / Math.max(negotiatedTotal, 1)) * 100, 1)}% do negociado` : "")}
        ${strategicHeroMetric("Valor negociado total", moneyCompact(negotiatedTotal), money(negotiatedTotal))}
        ${strategicHeroMetric("Concentração Top 5", `${number(topFiveShare, 1)}%`, `${moneyCompact(topFiveValue)} · ${money(topFiveValue)}`)}
      </div>
    </section>
    ...
    <div class="content-grid three strategic-chart-grid">
      <section class="panel">
        ...
        ${barList(historicalNegotiatedByRegion(), "valor", moneyCompact)}
      </section>
      <section class="panel">
        <div class="panel-header">
          <div>
      ...
      <section class="panel">
        ...
        ${barList(historicalClassificationRows(), "valor", (value) => String(value))}
      </section>
    </div>
