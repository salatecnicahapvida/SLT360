...
function matchesStrategicEVDecisionFilters(row) {
  ...
  const searchable = normalizeSearchText(`${row.record.project} ${row.record.code} ${row.record.typology} ${row.record.year}`);
...
function renderWorksStrategic() {
  ...
  const visibleDecisionRows = filteredDecisionRows.slice(0, 200);
  ...
  return `
    ...
    <section class="panel strategic-investment-panel strategic-intelligence-ranking">
      ...
      <div class="strategic-filter-summary"><strong>${number(filteredDecisionRows.length)} EVs encontrados</strong><span>Mostrando ${number(visibleDecisionRows.length)} na planilha</span><span>${moneyCompact(filteredDecisionValue)} em valor analisado</span><span>${number((filteredDecisionValue / Math.max(totalValue, 1)) * 100, 1)}% da base financeira</span></div>
