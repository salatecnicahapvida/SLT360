...
function evHistoricalFilteredRecords({ ignoreDiscipline = false } = {}) {
  ...
  return evHistoricalSourceRecords().filter((record) => {
    ...
    const haystack = normalizeSearchText(`${record.code || ""} ${record.project} ${record.revision} ${record.typology}`);
...
function renderEVHistoricalIntelligence() {
  ...
  return `
    <section class="panel ev-history-panel">
      <div class="panel-header ev-history-heading"><div><span class="eyebrow">Base de inteligência · 2020 a 2026</span><h2>Histórico de EVs por disciplina</h2><p class="panel-subtitle">${source.length} estudos importados de ${escapeAttribute(window.EV_HISTORICAL_DATA.source)}. Percentuais calculados sobre o EV sem taxa de risco.</p></div><span class="ev-history-badge">Base atualizada</span></div>
      ...
      <div class="ev-history-grid">
        ...
        <article class="ev-history-table-card"><div class="panel-header"><div><h3>Base histórica consultável</h3><p class="panel-subtitle">Abra o EV para consultar todas as filhas da coluna F</p></div><span class="tag">${records.length} EVs</span></div><div class="table-wrap ev-history-table-wrap"><table class="data-table"><thead><tr><th>Ano / EV</th><th>Tipologia</th><th class="numeric">Valor</th><th class="numeric">Área</th><th class="numeric">% disciplina</th><th>Ação</th></tr></thead><tbody>
          ${records.slice(0, 60).map((record) => { const disciplineValue = evHistoricalFilters.discipline ? Number(record.disciplines?.[evHistoricalFilters.discipline] || 0) : 0; const share = record.baseTotal ? (disciplineValue / record.baseTotal) * 100 : 0; return `<tr><td><button class="ev-history-project-link" type="button" data-action="open-historical-ev" data-id="${record.id}">${record.year} · ${escapeAttribute(record.project)}</button><br /><span class="muted">${escapeAttribute(record.code || "Sem código")} · ${escapeAttribute(record.revision)} · ${number(record.items?.length || 0)} filhas</span></td><td>${escapeAttribute(record.typology)}</td><td class="numeric">${moneyCompact(record.total)}</td><td class="numeric">${record.area ? `${number(record.area, 0)} m²` : "—"}</td><td class="numeric">${evHistoricalFilters.discipline ? `${number(share, 1)}%` : "Selecione"}</td><td><button class="secondary-action compact-action" type="button" data-action="open-historical-ev" data-id="${record.id}">Abrir EV</button></td></tr>`; }).join("") || `<tr><td colspan="6"><div class="empty-state">Nenhum EV histórico encontrado.</div></td></tr>`}
  ...
  modalRoot.innerHTML = `
    <div class="modal-backdrop" data-action="close-modal">
      <article class="modal-card ev-historical-modal" aria-labelledby="historicalEVTitle">
        ...
        <footer class="modal-actions"><span class="muted">Fonte: ${escapeAttribute(window.EV_HISTORICAL_DATA.source)} · Planilha1 · coluna F</span><button class="primary-action" type="button" data-action="close-modal">Fechar EV</button></footer>
      </article>
    </div>`;
}
