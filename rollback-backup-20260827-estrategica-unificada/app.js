  ...
  return `
    ...
    <section class="strategic-hero panel">
      ...
      <div class="strategic-hero-metrics">
        ${strategicHeroMetric("CAPEX total", moneyCompact(capex), money(capex))}
        ${strategicHeroMetric("Maior investimento", topInvestment ? topInvestment.work.nome : "Sem carteira", topInvestment ? `${moneyCompact(topInvestment.valor)} · ${number((topInvestment.valor / Math.max(capex, 1)) * 100, 1)}% do CAPEX` : "")}
        ${strategicHeroMetric("Valor do maior EV", topInvestment ? moneyCompact(topInvestment.valor) : money(0), topInvestment ? money(topInvestment.valor) : "")}
    ...
    <section class="strategic-kpis">
      ${executiveKpi("CAPEX total", moneyCompact(capex), money(capex), "EVs consolidados no portfólio atual", "blue", "strategicCapex", "CAPEX")}
      ${executiveKpi("Sala Técnica histórica", moneyCompact(commissionSummary.valorSalaTecnica), money(commissionSummary.valorSalaTecnica), `${commissionSummary.count} obras com leitura válida`, "blue", "strategicCommissionBase", "ST")}
      ${executiveKpi("Negociado histórico", moneyCompact(commissionSummary.valorNegociado), money(commissionSummary.valorNegociado), `Saving de ${moneyCompact(commissionSummary.savingTecnico)}`, "green", "strategicCommissionBase", "NEG")}
      ${executiveKpi("Área histórica", metricCompact(commissionSummary.area, " m²"), `${number(commissionSummary.area, 2)} m²`, "Área construída com leitura válida", "blue", "strategicCommissionBase", "ÁREA")}
      ...
      ${executiveKpi("EVs pendentes", String(pendingEvs), `${state.works.length} obras no portfólio`, "Risco para consolidação do CAPEX", pendingEvs ? "red" : "green", "strategicPendingEvs", "RISCO")}
      ${executiveKpi("Saldo crítico", String(criticalBalance), criticalBalance ? "Ação imediata necessária" : "Nenhum projeto no limite", "Projetos próximos ao limite financeiro", criticalBalance ? "red" : "green", "strategicCriticalBalance", "SALDO")}
      ...
      ${executiveKpi("Obras estratégicas", String(strategicWorks), `${number((strategicWorks / Math.max(state.works.length, 1)) * 100, 1)}% do portfólio`, "Classificação estratégica", "blue", "strategicWorks", "PORTFÓLIO")}
      ${executiveKpi("Novas unidades", String(newUnits), `${number((newUnits / Math.max(state.works.length, 1)) * 100, 1)}% do portfólio`, "Expansão de rede", "green", "strategicNewUnits", "EXPANSÃO")}
      ...
      ${executiveKpi("Saving histórico", moneyCompact(commissionSummary.savingTecnico), money(commissionSummary.savingTecnico), "Sala Técnica menos valor negociado", commissionSummary.savingTecnico >= 0 ? "green" : "red", "strategicCommissionBase", "SAVING")}
    </section>
