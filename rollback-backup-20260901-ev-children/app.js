  ...
  return `
    <section class="panel ev-history-panel">
      ...
      <div class="ev-history-grid">
        ...
        <article class="ev-history-table-card"><div class="panel-header"><div><h3>Base histórica consultável</h3><p class="panel-subtitle">Valores, área e disciplina selecionada</p></div><span class="tag">${records.length} EVs</span></div><div class="table-wrap ev-history-table-wrap"><table class="data-table"><thead><tr><th>Ano / EV</th><th>Tipologia</th><th class="numeric">Valor</th><th class="numeric">Área</th><th class="numeric">% disciplina</th></tr></thead><tbody>
          ${records.slice(0, 60).map((record) => { const disciplineValue = evHistoricalFilters.discipline ? Number(record.disciplines?.[evHistoricalFilters.discipline] || 0) : 0; const share = record.baseTotal ? (disciplineValue / record.baseTotal) * 100 : 0; return `<tr><td><strong>${record.year} · ${escapeAttribute(record.project)}</strong><br /><span class="muted">${escapeAttribute(record.code || "Sem código")} · ${escapeAttribute(record.revision)}</span></td><td>${escapeAttribute(record.typology)}</td><td class="numeric">${moneyCompact(record.total)}</td><td class="numeric">${record.area ? `${number(record.area, 0)} m²` : "—"}</td><td class="numeric">${evHistoricalFilters.discipline ? `${number(share, 1)}%` : "Selecione"}</td></tr>`; }).join("") || `<tr><td colspan="5"><div class="empty-state">Nenhum EV histórico encontrado.</div></td></tr>`}
