...
test('historical EV shows original and additive totals with an unobstructed title',async({page})=>{
 ...
 await row.getByRole('button',{name:'Visualizar EV',exact:true}).click();
 ...
 await safeRow.getByRole('button',{name:'Visualizar EV',exact:true}).click();
...
test('portfolio rows open options for the exact current or historical EV and handle works without EV',async({page})=>{
 ...
 await options.getByRole('button',{name:'Visualizar EV',exact:true}).click();
 ...
 await expect(options.getByRole('button',{name:'Visualizar EV'})).toHaveCount(0);
 await options.getByRole('button',{name:'Fechar'}).click();
 await current.getByRole('button',{name:'Visualizar EV',exact:true}).click();
 await expect(page.locator('#evModalTitle')).toHaveText('Obra de teste');
...
test('portfolio includes works without EV, selectable filters and the single requested KPI',async({page})=>{
 ...
 await expect(historicalRow.getByRole('button',{name:'Visualizar EV'})).toBeVisible();
 await expect(historicalRow.getByRole('button',{name:'Editar EV'})).toBeVisible();
 await expect(historicalRow.getByRole('button',{name:'Reajustar INCC'})).toBeVisible();
 ...
 await historicalRow.getByRole('button',{name:'Visualizar EV'}).click();
 ...
 await expect(noEvRow.getByRole('button',{name:'Visualizar EV'})).toHaveCount(0);
