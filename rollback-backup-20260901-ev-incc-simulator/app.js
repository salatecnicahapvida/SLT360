...
let sltINCCCalculator = { value: 0, baseYear: "2023" };
...
function renderWorksStrategic() {
  const sourceRecords = historicalWorksRecords();
  const historicalSummary = historicalWorksSummary(sourceRecords);
  const capex = historicalSummary.salaTecnica;
  const negotiatedTotal = historicalSummary.negociado;
  const regions = new Set(sourceRecords.map((record) => record.regiao).filter(Boolean)).size;
  const strategicWorks = sourceRecords.filter((record) => {
    const text = normalizeSearchText(`${record.classificacaoObra} ${record.tipologiaObra} ${record.nomeObra}`);
    return text.includes("estrateg") || text.includes("expans") || text.includes("vertical");
  }).length;
  const newUnits = sourceRecords.filter((record) => normalizeSearchText(`${record.tipologiaObra} ${record.tipoObra2} ${record.nomeObra}`).match(/\b(nova|novo)\b/)).length;
  const pendingWorks = sourceRecords.filter((record) => !normalizeSearchText(record.status).includes("concluida")).length;
  const unmeasuredWorks = sourceRecords.filter((record) => !record.leituraValidaM2).length;
  const investmentRows = historicalInvestmentRankingRows();
  const topInvestment = investmentRows[0];
  const topFiveValue = investmentRows.slice(0, 5).reduce((sum, row) => sum + row.valor, 0);
  const topFiveShare = negotiatedTotal ? (topFiveValue / negotiatedTotal) * 100 : 0;
  const targetRows = strategicCostTargetRows();
  const targetSummary = strategicCostTargetSummary(targetRows);
  ...
  return `
    ${renderWorksToolbar("worksStrategic", "Visão Estratégica", `Base consolidada de ${sourceRecords.length} obras · ${globalThis.GENERAL_WORKS_DATA?.source || "Base Geral"}`, `
    ...
    <section class="strategic-hero panel">
      ...
      <div class="strategic-hero-metrics">
        ${strategicHeroMetric("Sala Técnica total", moneyCompact(capex), money(capex), "ST", "blue", 100)}
        ${strategicHeroMetric("Maior obra negociada", topInvestment ? topInvestment.record.nomeObra : "Sem carteira", topInvestment ? `${moneyCompact(topInvestment.valor)} · ${number((topInvestment.valor / Math.max(negotiatedTotal, 1)) * 100, 1)}% do negociado` : "", "TOP 1", "orange", topInvestment ? (topInvestment.valor / Math.max(negotiatedTotal, 1)) * 100 : 0)}
        ${strategicHeroMetric("Valor negociado total", moneyCompact(negotiatedTotal), money(negotiatedTotal), "NEG", "green", (negotiatedTotal / Math.max(capex, 1)) * 100)}
    ...
    <section class="strategic-kpis">
      ${executiveKpi("Sala Técnica total", moneyCompact(capex), money(capex), `${sourceRecords.length} obras da base atualizada`, "blue", "strategicCommissionBase", "ST")}
      ${executiveKpi("Negociado total", moneyCompact(negotiatedTotal), money(negotiatedTotal), `Saving de ${moneyCompact(historicalSummary.saving)}`, "green", "strategicCommissionBase", "NEG")}
      ${executiveKpi("Área construída", metricCompact(historicalSummary.area, " m²"), `${number(historicalSummary.area, 2)} m²`, "Área total informada na planilha", "blue", "strategicCommissionBase", "ÁREA")}
      ${executiveKpi("Custo ponderado m²", `${money(historicalSummary.precoM2)}/m²`, `${commissionSummary.count} leituras válidas`, "Negociado dividido pela área válida", "orange", "strategicCommissionBase", "R$/m²")}
      ${executiveKpi("Aderência à meta m²", targetSummary.measured ? `${number(targetSummary.adherence)}%` : "Sem leitura", `${targetSummary.within} de ${targetSummary.measured} obras válidas`, "Dentro da premissa por tipologia", targetTone, "strategicCostTargets", "META")}
      ${executiveKpi("Acima da meta m²", String(targetSummary.above), `${targetSummary.measured} obras medidas`, "Projetos acima da premissa de custo", targetSummary.above ? "red" : "green", "strategicAboveTarget", "ALERTA")}
      ${executiveKpi("Obras não concluídas", String(pendingWorks), `${sourceRecords.length} obras analisadas`, "Status diferente de concluída na planilha", pendingWorks ? "orange" : "green", "strategicCommissionBase", "STATUS")}
      ${executiveKpi("Sem leitura de m²", String(unmeasuredWorks), `${commissionSummary.count} leituras válidas`, "Registros sem área ou custo por m² válido", unmeasuredWorks ? "orange" : "green", "strategicCommissionBase", "QUALIDADE")}
      ${executiveKpi("Regiões atendidas", String(regions), "Cobertura nacional do portfólio", "Distribuição territorial das obras", "green", "strategicRegions", "REDE")}
      ${executiveKpi("Obras estratégicas", String(strategicWorks), `${number((strategicWorks / Math.max(sourceRecords.length, 1)) * 100, 1)}% da base`, "Classificação estratégica, expansão ou verticalização", "blue", "strategicCommissionBase", "PORTFÓLIO")}
      ${executiveKpi("Novas unidades", String(newUnits), `${number((newUnits / Math.max(sourceRecords.length, 1)) * 100, 1)}% da base`, "Registros identificados como nova unidade", "green", "strategicCommissionBase", "EXPANSÃO")}
      ${executiveKpi("Concentração Top 5", `${number(topFiveShare, 1)}%`, `${moneyCompact(topFiveValue)} · ${money(topFiveValue)}`, "Participação dos cinco maiores EVs", topFiveShare > 80 ? "orange" : "blue", "strategicTop5", "CONCENTRAÇÃO")}
      ${executiveKpi("Base histórica m²", commissionSummary.count ? `${money(commissionSummary.precoM2Mediana || commissionSummary.precoM2Ponderado)}/m²` : "Sem base", `${commissionSummary.count} registros válidos`, "Mediana histórica de construção", "blue", "strategicCommissionBase", "BENCHMARK")}
      ${executiveKpi("Saving total", moneyCompact(historicalSummary.saving), money(historicalSummary.saving), "Sala Técnica menos valor negociado", historicalSummary.saving >= 0 ? "green" : "red", "strategicCommissionBase", "SAVING")}
    </section>

    ${renderHistoricalWorksAssistant()}

    <div class="content-grid three strategic-chart-grid">
      <section class="panel strategic-visual-panel region-visual-panel">
        <div class="panel-header">
          <div>
            <h2>Valor negociado por região</h2>
            <p class="panel-subtitle">Distribuição das ${sourceRecords.length} obras da Base Geral</p>
          </div>
        </div>
        ${renderHistoricalRegionalChart()}
      </section>
      <section class="panel strategic-visual-panel target-visual-panel">
        ...
        ${renderHistoricalTargetChart(targetRows)}
      </section>
      <section class="panel strategic-visual-panel classification-visual-panel">
        ...
        ${renderHistoricalClassificationChart()}
      </section>
    </div>
    ...
    <section class="panel strategic-investment-panel">
      <div class="panel-header">
        <div>
          <h2>Maiores valores negociados</h2>
          <p class="panel-subtitle">Ranking das obras da Base Geral, com custo por m² e participação no total negociado</p>
        </div>
        <span class="historical-source-badge">${sourceRecords.length} obras analisadas</span>
      </div>
      ${renderHistoricalInvestmentRanking()}
    </section>
  `;
}
...
function evUnifiedRecords() {
  ...
  const historicalRows = evHistoricalSourceRecords().map((record) => {
    ...
    return { ...record, sourceKind: "historical", workId: work?.id || "", sourceLabel: work ? "Histórico + cadastro" : "Histórico", searchAliases: work?.nome || "" };
  });
  const currentRows = state.works
    ...
    .map((work) => {
      ...
      return {
        ...
        year: date ? String(date).slice(0, 4) : "Atual", typology: evHistoricalTypologyForWork(work) || work.tipologiaObra || work.tipoUnidade || "Não informada",
  ...
  modalRoot.innerHTML = `
    <div class="modal-backdrop" data-action="close-modal">
      <article class="modal-card ev-historical-modal" aria-labelledby="historicalEVTitle">
        ...
        <footer class="modal-actions"><span class="muted">Fonte: ${escapeAttribute(window.EV_HISTORICAL_DATA.source)} · Planilha1 · coluna F</span><div class="table-actions">${Number(record.year) <= 2025 ? `<button class="ghost-button" type="button" data-action="load-ev-incc" data-id="${record.id}">Atualizar pelo INCC</button>` : ""}<button class="secondary-action" type="button" data-action="close-modal">Fechar composição</button><button class="primary-action" type="button" data-action="edit-historical-ev" data-id="${record.id}">Editar EV</button></div></footer>
      </article>
    </div>`;
}
...
function sltINCCFactor(baseYear) {
  ...
  sltINCCData.monthly2026.forEach((rate) => { factor *= 1 + rate / 100; });
  return factor;
}

function sltINCCReading(value = sltINCCCalculator.value, baseYear = sltINCCCalculator.baseYear) {
  const amount = Number(value) || 0;
  const factor = sltINCCFactor(baseYear);
...
function renderSLTCalculator() {
  ...
  return `
    <section class="panel slt-calculator" id="sltCalculator">
      <div class="panel-header"><div><span class="eyebrow">Calculadoras SLT</span><h2>Atualização de valor por INCC-M</h2><p class="panel-subtitle">Atualização composta da referência de dezembro do ano-base até ${sltINCCData.latestLabel}.</p></div><a class="secondary-action" href="${sltINCCData.sourceUrl}" target="_blank" rel="noopener noreferrer">Consultar fonte FGV</a></div>
      <div class="slt-calculator-layout">
        <div class="slt-calculator-form">
          ...
          <label class="field"><span>Ano-base do EV</span><select data-slt-incc-year>${[2020,2021,2022,2023,2024,2025].map((year) => `<option value="${year}" ${String(year) === String(sltINCCCalculator.baseYear) ? "selected" : ""}>dez/${year}</option>`).join("")}</select></label>
...
function loadHistoricalEVIntoINCC(recordId) {
  const record = evHistoricalSourceRecords().find((item) => item.id === recordId);
  if (!record) return;
  sltINCCCalculator = { value: Number(record.total || 0), baseYear: String(Math.min(Number(record.year) || 2025, 2025)) };
  closeModal();
  render();
  document.querySelector("#sltCalculator")?.scrollIntoView({ behavior: "smooth", block: "start" });
  showToast(`Valor do EV carregado na Calculadora SLT (${record.year}).`);
}
...
document.addEventListener("click", async (event) => {
  ...
  if (action === "load-ev-incc") {
    loadHistoricalEVIntoINCC(actionButton.dataset.id);
    return;
  }
...
document.addEventListener("change", (event) => {
  ...
  if (event.target.matches("[data-slt-incc-year]")) {
    sltINCCCalculator.baseYear = event.target.value;
