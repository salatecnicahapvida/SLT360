...
Após a publicação, o fluxo de entrada foi medido separadamente. A sessão, o perfil, as permissões, o catálogo e o backup eram aguardados em fases sucessivas. Essas consultas passaram a iniciar juntas, a listagem administrativa saiu da rota crítica e as três mensagens foram substituídas por uma única indicação de abertura. O carregador SQL também passou a combinar as entidades uma só vez: no conjunto remoto de 10.291 registros, o processamento medido caiu de aproximadamente 3,7 s para 2,7 s, com igualdade integral do JSON anterior e posterior verificada dentro de uma transação revertida.
