        ...
        <table class="data-table strategic-intelligence-table">
          ...
          <tbody>
            ${investmentRows.slice(0, 15).map((row, index) => {
              ...
              let tone = "green";
              if (!target) { reading = "Classificar meta do EV"; tone = "gray"; }
              else if (!costM2) { reading = "Sem área para validar R$/m²"; tone = "gray"; }
              else if (costM2 > target.targetMax) { reading = `Acima da meta em ${number(Math.abs(deviation), 0)}%`; tone = "red"; }
              else if (target.targetMin && costM2 < target.targetMin) { reading = `Abaixo da faixa em ${number(((target.targetMin / costM2) - 1) * 100, 0)}%`; tone = "orange"; }
