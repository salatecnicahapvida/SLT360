  ...
  return `
    <section class="panel historical-works-assistant">
      ...
      <div class="historical-summary-grid">
        ${splitItem("Obras encontradas", number(summary.count))}
        ${splitItem("Sala Técnica", money(summary.salaTecnica))}
        ${splitItem("Valor negociado", money(summary.negociado))}
        ${splitItem("Área construída", `${number(summary.area, 2)} m²`)}
        ${splitItem("Construção por m²", summary.precoM2 ? `${money(summary.precoM2)}/m²` : "Sem leitura")}
        ${splitItem("Saving na negociação", money(summary.saving))}
      </div>
          ...
          <tbody>
            ${visibleRows.length ? visibleRows.map((record) => `
              <tr>
                ...
                <td>${record.valorSalaTecnica == null ? "—" : money(record.valorSalaTecnica)}</td>
                <td><strong>${record.valorNegociado == null ? "—" : money(record.valorNegociado)}</strong></td>
                <td>${record.areaM2 == null ? "—" : `${number(record.areaM2, 2)} m²`}</td>
                <td>${record.precoM2 == null ? "—" : `${money(record.precoM2)}/m²`}</td>
                <td class="${Number(record.gapSalaTecnicaVsNegociado) <= 0 ? "historical-saving" : "historical-overrun"}">${record.gapSalaTecnicaVsNegociado == null ? "—" : money(record.gapSalaTecnicaVsNegociado)}</td>
